	...
	tests := []struct {
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (string, string) {
				...
				err := os.WriteFile(file1, []byte("model One {}"), 0o600)
				require.NoError(t, err)
				err = os.WriteFile(file2, []byte("model Two {}"), 0o600)
				require.NoError(t, err)
				return tmpDir, ""
			},
			expectedError: false,
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (string, string) {
				...
				err = os.WriteFile(file1, []byte("model Root {}"), 0o600)
				require.NoError(t, err)
				err = os.WriteFile(file2, []byte("model Nested {}"), 0o600)
				require.NoError(t, err)
				return tmpDir, ""
			},
			expectedError: false,
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (string, string) {
				...
				err := os.WriteFile(visibleFile, []byte("model Visible {}"), 0o600)
				require.NoError(t, err)
				err = os.WriteFile(hiddenFile, []byte("model Hidden {}"), 0o600)
				require.NoError(t, err)
				return tmpDir, ""
			},
			expectedError: false,
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (string, string) {
				...
				err := os.WriteFile(dgFile, []byte("model Test {}"), 0o600)
				require.NoError(t, err)
				err = os.WriteFile(txtFile, []byte("README"), 0o600)
				require.NoError(t, err)
				return tmpDir, ""
			},
			expectedError: false,
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (string, string) {
				...
				err := os.WriteFile(badFile, []byte("model Test {}"), 0o600)
				require.NoError(t, err)
				return tmpDir, ""
			},
			expectedError: true,
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (string, string) {
				...
				err = os.WriteFile(file, []byte("model Nested {}"), 0o600)
				require.NoError(t, err)
	...
	tests := []struct {
		...
		},
		{
			...
			setupFunc: func(t *testing.T) (string, string) {
				...
				err := os.WriteFile(file, []byte("model Test {}"), 0o600)
				require.NoError(t, err)
				return tmpDir, ""
			},
			expectedError: false,
...
func TestDgDirStructureModelPaths(t *testing.T) {
	t.Run("model path delimiter conversion", func(t *testing.T) {
		...
		err = os.WriteFile(file, []byte("model User {}"), 0o600)
		require.NoError(t, err)
...
func TestDgDirModelCount(t *testing.T) {
	t.Run("count models in nested structure", func(t *testing.T) {
		...
		err := os.WriteFile(rootFile, []byte("model Root {}"), 0o600)
		require.NoError(t, err)
		...
		err = os.WriteFile(file1, []byte("model M1 {}"), 0o600)
		require.NoError(t, err)
		err = os.WriteFile(file2, []byte("model M2 {}"), 0o600)
		require.NoError(t, err)
		err = os.WriteFile(file3, []byte("model M3 {}"), 0o600)
		require.NoError(t, err)
...
func TestGetDgDirectoryStructureComplexPaths(t *testing.T) {
	t.Run("complex nested paths with multiple models per directory", func(t *testing.T) {
		...
		err := os.MkdirAll(apiV1, 0o750)
		require.NoError(t, err)
		err = os.MkdirAll(apiV2, 0o750)
		require.NoError(t, err)
		err = os.MkdirAll(internal, 0o750)
		require.NoError(t, err)

		// Add multiple models in api/v1
		err = os.WriteFile(filepath.Join(apiV1, "User.dg"), []byte("model User {}"), 0o600)
		require.NoError(t, err)
		err = os.WriteFile(filepath.Join(apiV1, "Order.dg"), []byte("model Order {}"), 0o600)
		require.NoError(t, err)

		// Add models in api/v2
		err = os.WriteFile(filepath.Join(apiV2, "Product.dg"), []byte("model Product {}"), 0o600)
		require.NoError(t, err)

		// Add models in internal/models
		err = os.WriteFile(filepath.Join(internal, "Config.dg"), []byte("model Config {}"), 0o600)
		require.NoError(t, err)
		err = os.WriteFile(filepath.Join(internal, "Settings.dg"), []byte("model Settings {}"), 0o600)
		require.NoError(t, err)
	...
	t.Run("single file in deeply nested cumulated path", func(t *testing.T) {
		...
		err = os.WriteFile(filepath.Join(nestedPath, "Deep.dg"), []byte("model Deep {}"), 0o600)
		require.NoError(t, err)
...
func TestGetDgFileStructureEdgeCases(t *testing.T) {
	...
	t.Run("large .dg file", func(t *testing.T) {
		...
		err := os.WriteFile(file, []byte(largeModel), 0o600)
		require.NoError(t, err)
...
func TestGetDgDirectoryStructureWithMixedContent(t *testing.T) {
	t.Run("directory with .dg files and subdirectories", func(t *testing.T) {
		...
		err := os.WriteFile(filepath.Join(tmpDir, "Root.dg"), []byte("model Root {}"), 0o600)
		require.NoError(t, err)
		...
		err = os.Mkdir(subDir, 0o750)
		require.NoError(t, err)
		err = os.WriteFile(filepath.Join(subDir, "Sub.dg"), []byte("model Sub {}"), 0o600)
		require.NoError(t, err)
	...
	t.Run("hidden directories should be ignored", func(t *testing.T) {
		...
		// Create visible file
		err := os.WriteFile(filepath.Join(tmpDir, "Visible.dg"), []byte("model Visible {}"), 0o600)
		require.NoError(t, err)

		// Create hidden directory with files
		hiddenDir := filepath.Join(tmpDir, ".hidden")
		err = os.Mkdir(hiddenDir, 0o750)
		require.NoError(t, err)
		err = os.WriteFile(filepath.Join(hiddenDir, "Hidden.dg"), []byte("model Hidden {}"), 0o600)
		require.NoError(t, err)
...
func TestGetDgDirStructureWithBothFileAndDirectory(t *testing.T) {
	t.Run("input that is both file and directory parent", func(t *testing.T) {
		...
		err := os.WriteFile(file, []byte("model Single {}"), 0o600)
		require.NoError(t, err)
