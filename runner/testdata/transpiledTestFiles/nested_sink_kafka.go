...
// Sink_kafka___datagen_nested_data loads __datagen_nested data into Kafka
func Sink_kafka___datagen_nested_data(modelName string, records []*__datagen_nested, config *__dgi_KafkaConfig) error {
	slog.Debug(fmt.Sprintf("initializing Kafka producer for %s with %d records", modelName, len(records)))
	if err := Init___datagen_nested_kafka_producer(config); err != nil {
		return fmt.Errorf("✘ [Kafka] %s: FAILED\n   └─ Messages sent: 0/%d\n   └─ Error: %v\n",
			modelName, len(records), err)
	}
	defer func() {
		err := Close___datagen_nested_kafka_producer()
		if err != nil {
			slog.Warn(fmt.Sprintf("failed to close Kafka producer for %s: %s", modelName, err.Error()))
		}
	}()

	_, err := Get___datagen_nested_kafka_producer()
	if err != nil {
		return fmt.Errorf("✘ [Kafka] %s: FAILED\n   └─ Messages sent: 0/%d\n   └─ Error: %v\n",
			modelName, len(records), err)
	}
	...
	for i := 0; i < len(records); i += batchSize {
		...
		if err := Load___datagen_nested_kafka(batch, config); err != nil {
			return fmt.Errorf("✘ [Kafka] %s: FAILED\n   └─ Messages sent: %d/%d\n   └─ Error: %v\n",
...
// Clear_kafka___datagen_nested_data is a no-op for Kafka as it's an append-only log
func Clear_kafka___datagen_nested_data(modelName string, config *__dgi_KafkaConfig) error {
