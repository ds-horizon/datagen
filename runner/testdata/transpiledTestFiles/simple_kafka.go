...
func __datagen_simple_GetFieldAsString(record *__datagen_simple, fieldName string) (string, error) {
	switch fieldName {
	case "id":
	...
	default:
		return "", fmt.Errorf("field '%s' not found in simple", fieldName)
	}
}

// Load___datagen_simple_kafka sends a batch of records to the configured Kafka topic.
func Load___datagen_simple_kafka(records []*__datagen_simple, config *__dgi_KafkaConfig) error {
	if len(records) == 0 {
		return nil
	}

	producer, err := Get___datagen_simple_kafka_producer()
	if err != nil {
	...
	for _, record := range records {
		...
		if config.Key != "" {
			keyValue, err := __datagen_simple_GetFieldAsString(record, config.Key)
			if err != nil {
...
// Truncate___datagen_simple_kafka is a no-op for Kafka as it's an append-only log.
func Truncate___datagen_simple_kafka(config *__dgi_KafkaConfig) error {
	return nil
}
