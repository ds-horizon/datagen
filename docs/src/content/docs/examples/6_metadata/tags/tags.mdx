...
Create a file named `users.dg` in the same directory:
...
```shell
$ datagenc gen . -t service=delivery
```
...
```shell
$ datagenc gen . -t service=user,team=Slytherin
```
...
```shell
$ datagenc gen . -t service=delivery,team=user
```
